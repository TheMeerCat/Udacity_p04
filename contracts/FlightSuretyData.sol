...
contract FlightSuretyData {
    ...
    mapping(address => uint256) private payouts; // Mapping passenfer -> how much he receives
    mapping(address => Airline) private registeredAirlines; // Contracts allowed to access data
    mapping(address => EnumerableSet.AddressSet) private airlineToVoters; // Contracts allowed to access data

    mapping(bytes32 => Insurance[]) private boughtInsurances; // Contracts allowed to access data
    ...
    function isApprovedAirline(address airlineAddress)
        ...
        returns (bool)
    {
        return EnumerableSet.contains(approvedAirlines, airlineAddress) && registeredAirlines[airlineAddress].fundedValue >= 10 ether;
    }

    function isUniqueVote(address forAirline, address fromAirline)
        external
        view
        returns (bool)
    {
 ...
 function isPayoutAvailable(address passenger)
        external
        view
        returns (bool)
    {
    ...
    modifier requireApprovedAirline(address airlineAddress) {
        require(
            EnumerableSet.contains(approvedAirlines, airlineAddress) && registeredAirlines[airlineAddress].fundedValue >= 10 ether,
    ...
    function voteForAirline(address forAirline, address fromAirline)
        external
