...
contract('Flight Surety Tests', async (accounts) => {
    ...
    it('(airline) can register Airline its not approved', async () => {
        ...
        assert.equal(isApprovedAirline, false, "Airline should not be able to register an approved airline");
    ...
    it("buy insurance for a flight", async () => {
        ...
        await config.flightSuretyApp.buyInsurance(AIRLINE_2, flightNo, timestamp, { from: PASSENGER_1, value: ONE_ETHER });

        let insurances = await config.flightSuretyData.getInsurance(AIRLINE_2, flightNo, timestamp, PASSENGER_1);
    ...
    it("test credit all insurees for delayed flight", async () => {
        ...
        await config.flightSuretyApp.buyInsurance(AIRLINE_2, flightNo, timestamp, { from: PASSENGER_2, value: ONE_ETHER });
        await config.flightSuretyData.authorizeContract(accounts[0]);
        await config.flightSuretyData.creditInsurees(AIRLINE_2, flightNo, timestamp);
